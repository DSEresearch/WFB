...
def override_temporal_grads(model, aux, mask, temporal_mode="laplacian", lambda_laplacian=1e-5):
    ...
    at_grad = grad_At, valid
    omega_grad = grad_omega, valid
    theta_t_grad = grad_theta_t, valid
...
parser.add_argument("--lambda_laplacian", type=float, default=1e-4)
...
model_specs = [
    ("FFN", FFNStrict(len(vocab), embed_dim=args.embed_dim), "standard"),
